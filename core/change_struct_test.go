...
func TestChangeStruct(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				transferConfStr: `{"dup_key1":"$.key1","dup_key2":"$.key2","dup_key3":{"dup_key4":"$.key1","dup_key5":"$.key2"},"dup_key6":["$.key1",{"dup_key7":"$.key1"}],"dup_key7":{"opr":"merge_map","data":["$.key3",{"dup_key8":"$.key1"}]},"dup_key9":{"opr":"muti_source","data":["$.key5","$.key2"]},"dup_key10":{"opr":"default_value","data":"10"},"dup_key11":{"opr":"merge_slice","data":["$.key5",["$.key1","$.key2",{"dup_key12":"$.key1"}]]},"dup_key13":{"opr":"change_type","data":"$.key1","to_type":"string"},"dup_key14":{"opr":"repeat_get","data":["$.key6","$.key7"]}}`,
				transferTarget:  `{"key1":1,"key2":2,"key3":{"key4":4},"key5":[1,2,3],"key6":"{\"key7\":7}"}`,
				jsonPathType:    jsonpath_type.JsonPathTypeOneLevel,
			},
			want:    `{"dup_key1":1,"dup_key10":"10","dup_key11":[1,2,3,1,2,{"dup_key12":1}],"dup_key13":"1","dup_key14":7,"dup_key2":2,"dup_key3":{"dup_key4":1,"dup_key5":2},"dup_key6":[1,{"dup_key7":1}],"dup_key7":{"dup_key8":1,"key4":4},"dup_key9":[1,2,3]}`,
			wanterr: nil,
		},
		{
